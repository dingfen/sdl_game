...
// 	if (z > 5) {
// 		DLOG(INFO) << "Cannot use xbrz scaling with zoom factor > 5.";
// 		z = 1;
// 	}
...
// 	if (z == 0) {
// 		DLOG(INFO) << "Create an empty image";
// 		return dst;
// 	}

// 	if(surf == nullptr || dst == nullptr) {
// 		DLOG(INFO) << "Could not create surface to scale onto";
// 		return nullptr;
// 	}
...
surface adjust_surface_color(const surface &surf, int red, int green, int blue)
{
	...
	if(nsurf == nullptr) {
		DLOG(INFO) << "failed to make neutral surface";
		return nullptr;
	}
...
surface shadow_image(const surface &surf, int scale)
{
	...
	if(nsurf == nullptr) {
		DLOG(INFO) << "failed to blur the shadow surface";
		return nullptr;
	}
...
surface brighten_image(const surface &surf, int32_t amount)
{
	...
	if(nsurf == nullptr) {
		DLOG(INFO) << "could not make neutral surface...";
		return nullptr;
	}
...
surface mask_surface(const surface &surf, const surface &mask, bool* empty_result, const std::string& filename)
{
	...
	if (nsurf->w !=  nmask->w) {
		...
		DLOG(INFO) << ss.str();
		DLOG(INFO) << "It will not be masked, please use: "<< nmask->w << "x" << nmask->h;
		return nsurf;
	}
...
surface light_surface(const surface &surf, const surface &lightmap)
{
	...
	if (nsurf->w != lightmap->w) {
		...
		// This function is used on all hexes and usually only for that
		// so better keep it simple and efficient for the normal case
		DLOG(INFO) << "Detected an image with bad dimensions: " << nsurf->w << "x" << nsurf->h;
		DLOG(INFO) << "It will not be lighted, please use: "<< lightmap->w << "x" << lightmap->h;
		return nsurf;
	}
	{
		surface_lock lock(nsurf);
...
surface cut_surface(const surface &surf, const SDL_Rect& r)
{
	...
	if(res == nullptr) {
		DLOG(INFO) << "Could not create a new surface in cut_surface()";
		return nullptr;
	}
...
surface get_surface_portion(const surface &src, SDL_Rect &area)
{
	...
	if(dst == nullptr) {
		DLOG(INFO) << "Could not create a new surface in get_surface_portion()";
		return nullptr;
	}
